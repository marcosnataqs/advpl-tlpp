...
/*/{Protheus.doc} GP05R007

Pedidos x Faturamentos Sintético
...
@type function
/*/
User Function GP05R007() // U_GP05R007()
    Local cPerg := "GP05R007"

    AjustaSX1(cPerg)
	If Pergunte(cPerg, .T., "Pedidos x Faturamentos Sintético")
        ProcExport()
    EndIf
...
Static Function ProcExport()
    ...
    Private cWorkSheet := "PEDXFAT-SINT" + MV_PAR01
	Private cTableName := "Pedidos x Faturamentos Sintético " + MV_PAR01 + IIF(!Empty(cUF), cUF, "") + IIF(!Empty(cRepr), cRepr, "")
        ...
        While QRY1->(!EOF())
            ...
            For i := 1 To (Len(aHead)-3)
                If SubStr(aHead[i],1,3) $ "PED"
                    ...
                    nTotalPed += aRow[i]
                ElseIf SubStr(aHead[i],1,7) $ "CODCLI/CLIENTE"
                    nY := i
                    aRow[i] := &("QRY1->" + &("aHead[nY]"))
                EndIf
            Next i
            ...
            AADD(aDados, aRow)
            aDev := BuscaDev(SubStr(QRY1->CODCLI,1,6), SubStr(QRY1->CODCLI,7,2))
            If Len(aDev) > 0
                AADD(aDados, aDev)
                aDev := {}
            EndIf
	...
	If BuscaFat()
        ...
        While QRY->(!EOF())
            For nH := 1 To Len(aDados)
                If QRY->CODCLI $ aDados[nH][1]
                    For i := 1 To (Len(aHead)-3)
                        If aDados[nH][2] <> "DEVOLUCOES"
...
Static Function BuscaFat()
    ...
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '01' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_JANEIRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '02' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_FEVEREIRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '03' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_MARCO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '04' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_ABRIL, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '05' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_MAIO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '06' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_JUNHO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '07' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_JULHO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '08' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_AGOSTO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '09' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_SETEMBRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '10' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_OUTUBRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '11' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_NOVEMBRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D2_EMISSAO, 5, 2 ) = '12' THEN ( D2_TOTAL ) ELSE 0 END ), 4 ) FAT_DEZEMBRO " + CRLF
    ...
    cQry += "GROUP BY SA1.A1_COD, SA1.A1_LOJA, SA1.A1_NREDUZ " + CRLF
    cQry += "ORDER BY SA1.A1_NREDUZ " + CRLF
...
@type function
/*/
Static Function BuscaDev(cCli,cLoja)
    Local cQry
    Local aDev := {}
    ...
    // cQry += "    SD2.D2_COD COD, " + CRLF
    // cQry += "    SB1.B1_DESC DESCRI, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '01' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_JANEIRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '02' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_FEVEREIRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '03' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_MARCO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '04' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_ABRIL, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '05' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_MAIO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '06' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_JUNHO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '07' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_JULHO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '08' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_AGOSTO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '09' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_SETEMBRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '10' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_OUTUBRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '11' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_NOVEMBRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( D1_EMISSAO, 5, 2 ) = '12' THEN ( NVL(D1_TOTAL, 0) ) ELSE 0 END ), 4 ) DEV_DEZEMBRO " + CRLF
    cQry += "FROM "+ RetSqlName("SD2") +" SD2 " + CRLF
    cQry += "INNER JOIN "+ RetSqlName("SB1") +" SB1 " + CRLF
    cQry += "    ON SB1.D_E_L_E_T_ <> '*' " + CRLF
    cQry += "    AND SB1.B1_FILIAL = '"+ xFilial("SB1") +"' " + CRLF
...
Static Function BuscaPed()
    ...
    // cQry += "    SB1.B1_DESC DESCRI, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '01' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_JANEIRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '02' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_FEVEREIRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '03' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_MARCO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '04' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_ABRIL, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '05' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_MAIO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '06' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_JUNHO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '07' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_JULHO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '08' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_AGOSTO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '09' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_SETEMBRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '10' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_OUTUBRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '11' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_NOVEMBRO, " + CRLF
    cQry += "    ROUND( SUM ( CASE WHEN SUBSTR( SC5.C5_EMISSAO, 5, 2 ) = '12' THEN (SC6.C6_VALOR) ELSE 0 END ), 4 ) PED_DEZEMBRO " + CRLF
