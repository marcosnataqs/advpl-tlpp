...
/*/{Protheus.doc} GP05R003

Faturamento Anual Clientes
...
@type function
/*/
User Function GP05R003() // U_GP05R003()
    Local cPerg := "GP05R003"

    AjustaSX1(cPerg)
	If Pergunte(cPerg, .T., "Faturamento Anual Clientes")
        ProcExport()
    EndIf
...
Static Function ProcExport()
    ...
    Private cWorkSheet := "FATCLI" + MV_PAR01 + IIF(MV_PAR06 == MV_PAR07, "-" + MV_PAR06, "")
	Private cTableName := "Faturamento Anual Clientes " + MV_PAR01 + IIF(!Empty(cUF),cUF,"") + IIF(!Empty(cRepr),cRepr,"")
...
Static Function Head()
    ...
    For nX:= 1 to Len(aHead)
        If aHead[nX] $ "QTD_NF_PERIODO/QTD_MESES_FATURADO"
...
Static Function Body()
    ...
    For i := 1 To Len(aDados)
        ...
        nQtdNotas := BuscaNotas(aDados[i][3],SubStr(aDados[i][1],1,6),SubStr(aDados[i][1],7,2))

        aDados[i][41] := nQtdTotal
        aDados[i][42] := nSoma / nQtdTotal
        aDados[i][43] := nSoma
        aDados[i][44] := nQtdNotas
        aDados[i][45] := nMeses
        aDados[i][46] := nQtdTotal / nQtdNotas
        aDados[i][47] := nSoma / nQtdNotas
        ...
        If aDados[i][1] <> cCodCli
            ...
            aTotal  := Array(Len(aHead))
            For nX := 1 To Len(aDados[i])
                If SubStr(aHead[nX],1,3) == "VAL" .And. .Not. (aHead[nX] $ "VAL_FAT_MEDIO")
                	If Empty(aTotal[nX])
...
Static Function BuscaDados()
    ...
    MemoWrite("C:\Users\marcosnqs\Desktop\querys\GP05R003_BuscaDados.sql", cQry)
...
@type function
/*/
Static Function BuscaNotas(cProd,cCodCli,cLoja)
    Local cQry := ""
    Local nRet := 0
    ...
    cQry += "AND F2_VEND1 BETWEEN '"+ MV_PAR08 +"' AND '"+ MV_PAR09 +"' " + CRLF
    cQry += "AND F2_EST BETWEEN '"+ MV_PAR06 +"' AND '"+ MV_PAR07 +"' " + CRLF
    cQry := ChangeQuery(cQry)
