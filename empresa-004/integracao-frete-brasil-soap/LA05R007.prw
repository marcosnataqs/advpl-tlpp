...
/*/{Protheus.doc} LA05R007

Relatório de Ocorrências Extensivo
...
@type function
/*/
User Function LA05R007() //-- U_LA05R007()
	Local cPerg := "LA05R007"
	Private oReport
...
Static Function ReportDef()
	...
	Local cTitulo := "OCORRÊNCIAS EXTENSIVO"
	Local cDescricao := "Relatório de Ocorrências Extensivo - Apresenta todas ocorrências por nota."

	oReport := TReport():New("LA05R007",cTitulo,"LA05R007", {|oReport| PrintReport(oReport)}, cDescricao)
	oReport:SetLandscape()
	oReport:SetTotalInLine(.F.)

	oSectionInfo := TRSection():New(oReport, "OCORRÊNCIAS DE ENTREGAS - " + DTOC(MV_PAR07) + " a " + DTOC(MV_PAR08))
...
Static Function PrintReport(oReport)
	...
	While TMP1->(!EOF())
		...
		oSectionInfo:Cell("ZS_EMISSAO"):SetValue(STOD(TMP1->ZS_EMISSAO))
        oSectionInfo:Cell("ZS_NUMNF"):SetValue(TMP1->ZS_NUMNF)
        oSectionInfo:Cell("ZS_SERIE"):SetValue(TMP1->ZS_SERIE)
        oSectionInfo:Cell("ZS_PEDIDO"):SetValue(TMP1->ZS_PEDIDO)
        oSectionInfo:Cell("ZS_CLILOJ"):SetValue(TMP1->ZS_CLILOJ)
        oSectionInfo:Cell("ZS_CLINOME"):SetValue(TMP1->ZS_CLINOME)
        oSectionInfo:Cell("ZT_OCORREN"):SetValue(TMP1->ZT_OCORREN)
        oSectionInfo:Cell("ZT_CODIGO"):SetValue(TMP1->ZT_CODIGO)
        oSectionInfo:Cell("ZT_DESCRI"):SetValue(TMP1->ZT_DESCRI)
        oSectionInfo:Cell("ZT_OCORDAT"):SetValue(STOD(TMP1->ZT_OCORDAT))
        oSectionInfo:Cell("ZT_OCORHR"):SetValue(TMP1->ZT_OCORHR)
        oSectionInfo:Cell("ZT_TRANSRS"):SetValue(TMP1->ZT_TRANSRS)
